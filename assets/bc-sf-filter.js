...
var bcSfFilterTemplate = {
  ...
  // Sorting Template
  sortingHtml:
    '<label class="label--hidden">Sort</label><select class="collection-sort__input">{{sortingItems}}</select>',
};
...
BCSfFilter.prototype.buildProductGridItem = function (data, index) {
  ...
  for (var i = 0; i < data['variants'].length; i++) {
    ...
    variantNames +=
      ' ' + title.replace('5-', '').replace('3-', '').toLowerCase();
  }
  ...
  if (data.title != '') {
    ...
    if (onSale) {
      itemPriceHtml +=
        ...
        variantNames.replace('/', '') +
        '">' +
        this.formatMoney(data.compare_at_price_min, this.moneyFormat, true) +
        '</s> ';
    }
    if (priceVaries) {
      itemPriceHtml +=
        this.formatMoney(data.price_min, this.moneyFormat, true) +
        '<span class="icon-fallback-text"><span class="icon icon-plus grid-product__price-min" aria-hidden="true"></span><span class="fallback-text">+</span></span>';
    } else {
  ...
  // Add pack status
  var isPack =
    data.title.indexOf('Pack') != -1 ? bcSfFilterTemplate.isPack : '';
...
function buildItemImage(image, title) {
  ...
  html =
    '<img src="' +
    bcsffilter.optimizeImage(image, 'large') +
    '" ' +
    'srcset="' +
    bcsffilter.optimizeImage(image, 'small') +
    ' 100w,' +
    bcsffilter.optimizeImage(image, 'compact') +
    ' 160w,' +
    bcsffilter.optimizeImage(image, 'medium') +
    ' 240w,' +
    bcsffilter.optimizeImage(image, 'large') +
    ' 480w,' +
    bcsffilter.optimizeImage(image, 'grande') +
    ' 600w,' +
    bcsffilter.optimizeImage(image, '1024x1024') +
    ' 1024w" ' +
    'sizes="50vw" ' +
...
BCSfFilter.prototype.buildExtrasProductList = function (data, eventType) {
  ...
  if (
    jQ(this.getSelector('products') + ' > .grid__item.section-header').length ==
    0
  ) {
    ...
    if (
      typeof bcSfFilterConfig.custom.collection_desc !== 'undefined' &&
      bcSfFilterConfig.custom.collection_desc !== null &&
      bcSfFilterConfig.custom.collection_desc !== ''
    ) {
    ...
    collectionItemHtml = collectionItemHtml.replace(
      /{{itemTitle}}/g,
      collectionItemTitle
    );
    collectionItemHtml = collectionItemHtml.replace(
  ...
  this.buildExtrasProductListByAjax(
    data,
    'boost-integration',
    function (results) {
      results.forEach(function (result) {
        // Append the custom html to product item
        jQ('[data-product-id="' + result.id + '"] .boost-custom-html').html(
          result.custom_html
        );
      });
    }
...
BCSfFilter.prototype.buildAdditionalElements = function (data, eventType) {
  ...
  var braTopsHeaderHTML =
    '<div class="filter-header" id="bra-tops">Bra Tops</div>';
  ...
  var mixedHeaderHTML =
    '<div class="filter-header" id="everything-else">Everything Else</div>';
  ...
  var cottonUnderwearHeaderHTML =
    '<div class="filter-header" id="cotton-underwear">Cotton Underwear</div>';

  if (
    $('#underwear li a[data-title=XS-M]').length ||
    $('#thongs li a[data-title=XS-M]').length
  )
    $('li a[data-title=XS-M]').parent().before(cottonUnderwearHeaderHTML);
  else if (
    $('#underwear li a[data-title=M-XL]').length ||
    $('#thongs li a[data-title=M-XL]').length
  )
    $('li a[data-title=M-XL]').parent().before(cottonUnderwearHeaderHTML);

  // add IDs to inputs - first on the standard one
  $('#bc-sf-filter-tree .bc-sf-filter-option-multiple-list li input').attr(
    'id',
    function () {
  ...
  $('#bc-sf-filter-tree-h .bc-sf-filter-option-multiple-list li input').attr(
    'id',
    function () {
      var inputID = $(this).data('value');
      inputID = inputID.replace(/%3A/g, '-');
  ...
  $('.bc-sf-filter-option-multiple-list li').append(function () {
    ...
    var inputTitle =
      'Filter by ' + $input.data('parent-label') + ': ' + $input.data('title');

    return (
      '<label class="hide" for="' + inputID + '">' + inputTitle + '</label>'
    );
  });

  this.buildExtrasProductListByAjax(
    data.products,
    'boost-multiple-currencies',
    function (results) {
      results.forEach(function (result, index) {
        ...
        var maxPrice =
          ...
          '</s>';
        if (
          result.compare_at_price_min !== 0 &&
          result.price_min < result.compare_at_price_min
        ) {
          element
            .find('.grid-product__meta ')
            .append(
          ...
          element
            .find('.grid-product__meta ')
            .append(
              '<span class="grid-product__price">' + minPrice + '</span>'
            );
        }
      });
    }
...
BCSfFilter.prototype.buildPlaceholderProductList = function (data, eventType) {
  ...
  if (typeof isEnable !== 'undefined' && isEnable) {
    ...
    if (
      this.loadProductFirst == false ||
      this.isSearchPage() ||
      this.isVendorPage() ||
      this.isTagPage() ||
      this.defaultSorting == 'best-selling' ||
      this.getSettingValue('general.productAndVariantAvailable') == true ||
      (this.loadProductFirst == true &&
        (eventType != 'init' || window.location.search.length > 0))
    ) {
      //Check display is Grid tyle or this.buildPlaceholderProductExtra not defined
      if (
        typeof this.queryParams.display == 'undefined' ||
        this.queryParams.display == 'grid' ||
        typeof this.buildPlaceholderProductExtra != 'function'
      ) {
        var percent = 140,
          itemClass =
            bcSfFilterConfig.custom.product_grid_class ||
            this.getSettingValue('general.placeholderProductGridItemClass');
        ...
        if (itemClass == '') {
          if (jQ(this.getSelector('products')).find('> *').length > 0) {
            itemClass = jQ(this.getSelector('products'))
              .find('> *')
              .first()
              .attr('class');
          } else {
        ...
        if (
          ...
          (!this.isDefaultPaginationType() && eventType != 'page')
        ) {
          jQ(this.getSelector('products')).html('');
        }
        var products_per_row = parseInt(
          this.getSettingValue('general.placeholderProductPerRow')
        );
        ...
        for (var i = 0; i < products_per_row; i++) {
          if (
            jQ(this.getSelector('products') + ' > .grid__item.section-header')
              .length == 0 &&
            i == 0
          ) {
            var collectionItemHtml =
              bcSfFilterTemplate.collectionHeaderItemHtml;
            var collectionItemTitle =
              bcSfFilterConfig.custom.collection_title || '';
            var collectionItemDesc = '';
            if (
              typeof bcSfFilterConfig.custom.collection_desc !== 'undefined' &&
              bcSfFilterConfig.custom.collection_desc !== null &&
              ...
              if (
                bcSfFilterConfig.custom.collection_desc.indexOf('===') !== -1
              ) {
                var descArr =
                  bcSfFilterConfig.custom.collection_desc.split('===');
                var descF = descArr[0];
                var descS = descArr[1];
                collectionItemDesc = descF;
                var newDescS =
...
BCSfFilter.prototype.buildFilterOptionItem = function (
  ...
  if (fTreeType == 'vertical' || fTreeType == 'both') {
    ...
    if (
      this.getSettingValue('general.requestInstantly') ||
      fOType == 'collection'
    ) {
      var func =
        "onInteractWithFilterOptionValue(event, this, '" +
...
BCSfFilter.prototype.customizeFilterOptionLabel = function (
  label,
  prefix,
  filterType,
  ...
  if (this.getSettingValue('general.forceCapitalizeFilterOptionValues'))
    return capitalize(label, true);
  ...
  if (this.getSettingValue('general.capitalizeFirstLetterFilterOptionValues'))
    return capitalize(label, true, true);

  // Just capitalize first letter and don't change the format of any other letters
  // For example: hello wORLD => Hello WORLD
  if (this.getSettingValue('general.capitalizeFilterOptionValues'))
...
BCSfFilter.prototype.buildFilterOptionMultipleList = function (data) {
  ...
  if (
    ...
    (data.valueType == 'all' ||
      this.getSettingValue('general.sortManualValues') ||
      (data.valueType != 'all' && data.sortManualValues))
  )
    data.values = this.sortFilterOptionValue(data);
  ...
  for (var k in data.values) {
    ...
    if (typeof header != 'undefined') {
      if (
        verticalContent.indexOf('id="' + header + '"') < 0 &&
        horizontalContent.indexOf('id="' + header + '"') < 0
      ) {
        verticalContent +=
          '<div class="filter-header" id="' + header + '">' + header + '</div>';
        ...
        if (header == 'band') horizontalContent += 'Wired Bras';
        else if (header == 'size') horizontalContent += 'Non-Wire Bras';
        else horizontalContent += header;

        horizontalContent += '</div>';
      }
    }

    verticalContent += this.buildFilterOptionMultipleListData(
      fOType,
      fOId,
      fOLabel,
      fODisplayType,
    ...
    horizontalContent += this.buildFilterOptionMultipleListData(
      fOType,
      fOId,
      fOLabel,
      fODisplayType,
