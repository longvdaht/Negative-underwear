...
(function () {
  ...
  ProductGridItem.prototype.compileTemplate = function (data) {
    ...
    var variantNames = '';
        ...
        itemHtml = itemHtml.replace(/{{itemTitle}}/g, productTitle);
        itemHtml = itemHtml.replace(/{{itemHandle}}/g, data.handle);
        itemHtml = itemHtml.replace(/{{itemVendorLabel}}/g, data.vendor);
        itemHtml = itemHtml.replace(/{{itemUrl}}/g, Utils.buildProductItemUrlWithVariant(data));
        itemHtml = itemHtml.replace(/{{buildSwiperMobile}}/g, buildSwiperMobile(data));
    ...
    function buildSwiperMobile(data) {
      if (!data.images_info || data.images_info.length === 0) return '';
    
      let swiperHtml = '<div class="swiper-container swiper-mobile-negative"><div class="swiper-wrapper">';
    
      const desiredIndexes = [0, 1, 2];
    
      for (let i of desiredIndexes) {
        if (data.images_info[i]) {
          swiperHtml += `<div class="swiper-slide">
            <img src="${Utils.optimizeImage(data.images_info[i].src, 'large')}" alt="${data.title}">
          </div>`;
        }
      }
    ...
    function initSwiper() {
      setTimeout(() => {
        new Swiper('.swiper-mobile-negative', {
          slidesPerView: 1,
          spaceBetween: 10,
          pagination: {
    ...
    const observer = new MutationObserver(() => {
      if (document.querySelector('.swiper-mobile-negative')) {
        initSwiper();
        observer.disconnect();
      }
    });
  ...
  function PlpForm(product) {
    ...
    ProductList.prototype.afterRender = function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            const variantBadgesCustom = localeURLPart + '/products/' + data[i].handle + '?view=boost-pfs-variant-badges';
            $.ajax({
                url: variantBadgesCustom,
                success: function(result) {
                    $('[data-id="' + prodId + '"]').find('.grid-product__image-wrapper').append(result.trim());
                }
            });
        }
