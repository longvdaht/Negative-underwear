...
// Function to handle form submission on PLP
function onSubmitOnPlp(form) {
  let containerFormAll = document.querySelectorAll(form);
  if (containerFormAll) {
    containerFormAll.forEach((containerForm) => {
      const inputVariant = containerForm.querySelector('.product-variant-id');
      const btn = containerForm.querySelector('button.AddToCart');
      const productId = containerForm.getAttribute('data-product-id');

      // Event listener for input change to enable/disable the button
      containerForm.querySelectorAll('.ajax-label input').forEach((item) => {
        item.addEventListener('click', () => {
          inputVariant.value = item.getAttribute('data-variant');
          if (productId) {
            const variantBadges = document.querySelectorAll('[data-product-id="' + productId + '"].saletag.variant-badge');
            if (variantBadges && variantBadges.length) {
              variantBadges.forEach((badge) => {
                if (badge.getAttribute('data-variant-id') == item.getAttribute('data-variant')) {
          ...
          if (item.getAttribute('data-variant')) {
            btn.classList.remove('select-size-load');
            btn.setAttribute('type', 'submit');
            btn.disabled = false;
          } else {
            btn.disabled = true;
            btn.classList.add('select-size-load');
      ...
      containerForm.addEventListener('submit', (e) => {
        ...
        const variantId = inputVariant.value;
        if (!variantId) return;
        ...
        .then((res) => res.json())
        .then(() => {
          document.querySelector('.header__icon--cart.link.cart-link')?.click();
          containerForm.reset();
...
(() => {
  setTimeout(() => {
    const formToAddToCart = document.querySelectorAll(
      '.form.lgform.AddToCartForm'
    );

    for (const form of formToAddToCart) {
      const productId = form.getAttribute('data-product-id');
      onSubmitOnPlp(`form#product-form-main-product-ajax-${productId}`);
      handleOpenModalButton(productId);
    }
  }, 2000);
})();
...
class SliderComponent extends HTMLElement {
      ...
      if (this.querySelector('.slider-dots').innerHTML == false) {
        ...
        if (this.totalPages > 0) {
          for (var i = 1; i <= this.totalPages; i++) {
            ...
            var dotItem =
              '<button type="button" aria-label="Dots" name="dot" class="dotslide ' +
              active +
              '" data-position="' +
              i +
              '"></button>';
  ...
  onDotClick(event) {
    ...
    let currentPosition = parseInt(
      this.dotActive.getAttribute('data-position')
    );
    this.dotActive = event.currentTarget;
    event.currentTarget.classList.add('active');
    let slideScrollPosition =
      this.slider.scrollLeft +
      this.sliderItems[0].clientWidth * (number - currentPosition);
...
class VariantSelects extends HTMLElement {
  ...
  customSelect() {
    ...
    variantSelectWrapper.forEach((variantWrapper) => {
      ...
      if (customSelect) {
        ...
        const customSelectOptions = variantWrapper.querySelectorAll(
        ...
        customSelectOptions.forEach((option) => {
          option.addEventListener('click', function (e) {
            ...
            if (
              document.querySelector('.select-Size .select-selected')
                .textContent == window.variantStrings.customSelect
            )
              return;
  ...
  updateVariantInput() {
    ...
    if (currentVariantTitle.indexOf('-Pack') != -1) {
      var tempString = currentVariantTitle.substring(
        0,
        currentVariantTitle.indexOf('-Pack')
      );
    ...
    if (quantityChanger > 1) {
      var singleVariantTitle = currentVariantTitle
        .replace('2-Pack', 'Single')
        .replace('3-Pack', 'Single')
        .replace('5-Pack', 'Single');
      var singleVariantOptions = singleVariantTitle
        .replace(' / ', '/')
        .replace(' /', '/')
        .replace('/ ', '/')
        .split('/');
    ...
    if (
      this.currentVariant.title.indexOf('Single') != -1 &&
  ...
  renderProductInfo() {
    ...
    fetch(
      ...
      .then((responseText) => {
        ...
        const addButtonDestination = document.getElementById(
          `ProductSubmitButton-${sectionId}`
        );
          ...
          addButtonUpdated
            ? addButtonUpdated.hasAttribute('disabled') ||
                document
                  ...
                  .classList.contains('multi-soldout')
            : true,
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    const sectionElement = document.getElementById(
      `MainProduct-${this.dataset.section}`
    );
  ...
  soldOutStickyButton(
    inputs = [
      ...document.querySelectorAll(
        '.js-product-page variant-radios fieldset input'
      ),
    ]
  ) {
    ...
    if (disabled.length) {
      ...
      const fieldsetSticky = document.querySelectorAll(
        `.js-sticky-add-to-cart .variant-radios fieldset input`
      );
  ...
  updateOptions() {
    ...
    this.options = fieldsets.map((fieldset) => {
      return Array.from(fieldset.querySelectorAll('input')).find(
        (radio, i, array) => {
          if (radio.checked) {
            const inputSticky = document.querySelector(
              `.js-sticky-add-to-cart [name="Quantity"][value="${radio.value}"]`
            );
            const variantStickyfieldset = [
              ...document.querySelectorAll(
                '.js-sticky-add-to-cart .js.product-form__input input[type="hidden"][name="Quantity"]'
              ),
            ];
...
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll('label.sticky-atc-label-option').forEach(label => {
    const handleInteraction = (event) => {
        ...
        const stickyParent = label.closest('.product-form__input');
        const stickyInputs = stickyParent.querySelectorAll(`.pdp-input-option`);
        stickyInputs.forEach(stickyInput => {
            stickyInput.setAttribute('data-active', 'false');
        });

        const stickyInput = stickyParent.querySelector(`.js-sticky-add-to-cart .pdp-input-option[id^="${inputId}"]`);
        if (stickyInput && (stickyInput.getAttribute('data-active') !== 'true')) {
            stickyInput.setAttribute('data-active', 'true');
        }

        handleStickyCTA(label);
    };

    label.addEventListener('click', handleInteraction);
    label.addEventListener('touchstart', handleInteraction);
});


  document.querySelectorAll('.pdp-input-option').forEach(input => {
    input.addEventListener('focus', (event) => {
        event.preventDefault();
    });
  });

  function handleStickyCTA(label) {
    const stickyATC = document.querySelector('.sticky-cta-soldout-val');
    if (!isSizeOptWithMultipleVariants(label)) {
        if (label.classList.contains('sticky-cta-opt-soldout')) {
            stickyATC.setAttribute('disabled', 'disabled');
            stickyATC.querySelector('span').innerHTML = 'Sold Out';
        } else {
            stickyATC.removeAttribute('disabled');
            stickyATC.querySelector('span').innerHTML = 'Add to Cart';
        }
    }
  }

  function isSizeOptWithMultipleVariants(stickyLabel) {
    const stickyProductFormInput = stickyLabel.closest('.product-form__input');
    const stickyVariantRadios = stickyProductFormInput?.closest('.variant-radios');
    if (!stickyVariantRadios) return false;

    const stickyVisibleInputForms = Array.from(stickyVariantRadios.querySelectorAll('.product-form__input:not(.hide)'));

    if (stickyVisibleInputForms[0] === stickyProductFormInput && stickyVisibleInputForms.length > 1) {
      const fieldsets = Array.from(document.querySelectorAll('variant-radios fieldset.product-form__input:not(.hide)'));
      stickyVisibleInputForms.slice(1).forEach((stickyInputForm, index) => {
        const correspondingFieldset = fieldsets[index + 1];
        if (correspondingFieldset) {
          const pdpRadios = Array.from(correspondingFieldset.querySelectorAll('input[type="radio"]'));
          const stickyInputs = Array.from(stickyInputForm.querySelectorAll('input[type="hidden"]'));

          pdpRadios.forEach(pdpRadio => {
            const matchingHidden = stickyInputs.find(hidden => hidden.value === pdpRadio.value);

            if (matchingHidden) {
              const hasDisabled = pdpRadio.classList.contains('disabled');
              const hasHide = pdpRadio.classList.contains('hide');

              if (hasDisabled) {
                matchingHidden.classList.add('disabled');
              } else {
                matchingHidden.classList.remove('disabled');
              }

              if (hasHide) {
                matchingHidden.classList.add('hide');
              } else {
                matchingHidden.classList.remove('hide');
              }

              const nextLabel = matchingHidden.nextElementSibling;
              if (nextLabel && nextLabel.classList.contains('sticky-atc-label-option')) {
                if (hasDisabled || hasHide) {
                  nextLabel.classList.add('sticky-cta-opt-soldout');
                } else {
                  nextLabel.classList.remove('sticky-cta-opt-soldout');
                }
              }
            }
          });
          ...
          stickyInputs.forEach(stickyInput => {
            stickyInput.removeAttribute('data-active');
          })
        }
      });
      const stickyATC = document.querySelector('.sticky-cta-soldout-val');
      stickyATC.setAttribute('disabled', 'disabled');
      stickyATC.querySelector('span').innerHTML = 'Select Option';
      return true;
    }
    return false;
  }
  ...
  $.ajax({
    ...
    success: function (cart) {
      ...
      for (i in cart.items) {
        ...
        $('.ajaxcart__product[data-id=' + item.id + ']')
          .attr('data-handle', item.handle)
          .attr('data-size', item.variant_options[0]);
      }
      for (i in cart.items) {
        var item = cart.items[i];
        ...
        if (item.properties['_pack_handle']) {
          ...
          if (!checkedYet) {
            ...
            */
            ...
            if (productsToFind <= 0) {
              ...
              var phantomHtml = '';
              phantomHtml +=
                '<div class="ajaxcart__product" data-link="' +
                packUrl +
                '" data-id="' +
                packId +
                '" data-size="' +
              ...
              phantomHtml +=
                '        <a href="' +
                packUrl +
                '" class="ajaxcart__product-image"><img src="' +
                packImage +
                '" alt="' +
                packTitle +
                '"></a>';
              ...
              phantomHtml +=
                '          <a href="' +
                packUrl +
                '" class="ajaxcart__product-name z">' +
                packTitle +
                '</a>';
              phantomHtml +=
                '          <span class="ajaxcart__product-meta">Size: ' +
                packSize +
                '</span>';
              phantomHtml += '        </div>';
              ...
              phantomHtml +=
                '              <button type="button" class="ajaxcart__remove-pack" data-pack-id="' +
                packId +
                '" data-size="' +
                packSize +
                '" data-ids="' +
          ...
          */ phantomHtml += '          </div>';
              ...
              phantomHtml +=
                '                <small class="ajaxcart-item__price-strikethrough"><s>' +
                packOriginalPrice +
                '</s></small>';
              phantomHtml +=
                '                <div class="qb-price-item">' +
                packPrice +
                '</div>';
              phantomHtml += '              </div>';
              phantomHtml += '            </span>';
      ...
      if (false) {
        ...
        if (subtotal >= 500) {
          ...
          $(
            '.ajaxcart__product[data-link*=' + gwpHandle + '] .show-if-free'
          ).removeClass('hide');
          $('.ajaxcart__product[data-link*=' + gwpHandle + '] .qb-price-item')
            .html('FREE')
            .addClass('red');
  ...
  $('.ajaxcart__remove-pack').click(function () {
    ...
    for (var i = 0; i < cart.items.length; i++) {
      ...
      for (var j = 0; j < idsToRemove.length; j++) {
        ...
        if (
          item.id == idsToRemove[j] &&
          item.variant_options[0] == sizeToRemove
        ) {
          ...
          product.variants.forEach(function (variant) {
            if (variant.id == item.variant_id) {
              ...
              if (variant.compare_at_price != null) {
                ...
                $(
                  '.ajaxcart__product[data-id=' +
                    item.id +
                    '] .ajaxcart-item__price-strikethrough s'
                ).html(
                  Shopify.formatMoney(variant.compare_at_price).replace(
                    /((\,00)|(\.00))$/g,
                    ''
                  )
                );
                ...
                $(
                  ...
                  Shopify.formatMoney(variant.price).replace(
                    /((\,00)|(\.00))$/g,
                    ''
                  )
                );
                $(
                  '.ajaxcart__product[data-id=' + item.id + '] .qb-price-item'
                ).html(
                  Shopify.formatMoney(item.final_price).replace(
                    /((\,00)|(\.00))$/g,
                    ''
                  )
                );
